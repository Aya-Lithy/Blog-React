...
class Router extends Component {
  ...
  createComment = (comment) => {
    axios
      ...
      .then((res) => {
        if (res.status === 201) {
          Swal.fire("Comment Create", "It is created correctly.", "success");
    ...
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          const commentEdit = comments.findIndex(
            (comment) => commentId.id === comment.id
          );
            ...
            <Switch>
              ...
              <Route
                ...
                render={(props) => {
                  ...
                  commentFilter = comments.filter(
                    (comment) => comment.postId === Number(idPost)
                  );

                  return (
                    <div>
                      <SinglePost post={filter[0]} />
              ...
              <Route
                ...
                render={(props) => {
                  let idPost = props.location.pathname.replace(
                    "/editpost/",
                    ""
                  );
                  const posts = this.state.posts;
              ...
              <Route
                exact
                path="/post/:postId/editcomment/:commentId"
                render={(props) => {
                  ...
                  let idComment = props.location.pathname.replace(
                    totalPath,
                    ""
                  );
